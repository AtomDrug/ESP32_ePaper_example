...
static uint8_t GDOControl[] = {0x01, (EPD_DISPLAY_HEIGHT-1)%256, (EPD_DISPLAY_HEIGHT-1)/256, 0x00};
...
void initEPD()
{
	...
	//Buffers
	disp_buffer = (uint8_t*)malloc(EPD_DISPLAY_WIDTH * (EPD_DISPLAY_HEIGHT/8));
	drawBuff = disp_buffer;

	gs_disp_buffer = (uint8_t*)malloc(EPD_DISPLAY_WIDTH * EPD_DISPLAY_HEIGHT);
	gs_drawBuff = gs_disp_buffer;

	EPD_DisplayClearPart();

	//needed to init matrix
	EPD_fillScreen(_bg); 
...
static void EPD_Init()
{
	...
	EPD_SetRamArea(0x00, (EPD_DISPLAY_WIDTH-1)/8, EPD_DISPLAY_HEIGHT-1, 0);
	EPD_SetRamPointer(0x00, EPD_DISPLAY_HEIGHT-1);
#if EPD_DEBUG
	t1 = clock() - t1;
...
{
    EPD_SetRamPointer(0x00, EPD_DISPLAY_HEIGHT-1);	// set ram pointer
	if (type == 0){
		// Fill screen with white
		EPD_WriteDispRamMono(EPD_DISPLAY_WIDTH, EPD_DISPLAY_HEIGHT, 0xff);
	}
	else {
		// Fill screen from buffer
		EPD_WriteDispRam(EPD_DISPLAY_WIDTH, EPD_DISPLAY_HEIGHT, (uint8_t *)DisBuffer);
	}
	EPD_UpdateFull();
...
void EPD_DisplayClearPart()
{
	uint8_t m = 0xFF;
	EPD_init_Part();
	EPD_Dis_Part(0, EPD_DISPLAY_WIDTH-1, 0, EPD_DISPLAY_HEIGHT-1, &m, 0);	//all white
	m = 0x00;
	EPD_Dis_Part(0, EPD_DISPLAY_WIDTH-1, 0, EPD_DISPLAY_HEIGHT-1, &m, 0);	//all black
	m = 0xFF;
	EPD_Dis_Part(0, EPD_DISPLAY_WIDTH-1, 0, EPD_DISPLAY_HEIGHT-1, &m, 0);	//all white
